...
class runSimulation():
    ...
    def run(self):
        ...
        for edge in self.edges:
            # Unfortunately SUMO returns false edges from getIDList(), this will filter the false edges.
            self.calculateNeighbours(edge)
            self.calculateEdgeLengths(edge)
    ...
    def getEdgeDensity(self, edge_id):
        '''This is a value we will be able to predict in real life based on mean
        speed, gps position along the route, acceleration / deceleration data,
        average gap between vehicles, and possible data from other vehicles along
        the edge using the same app.'''
        vehicle_quantity = traci.edge.getLastStepVehicleNumber(edge_id)
        #print (vehicle_quantity)
    ...
    def calculateNeighbours(self, edge_id):
        ...
        node_1_ne = self.net.getNeighboringEdges(node_1_coord[0], node_1_coord[1], r=0.0001)
        node_2_ne = self.net.getNeighboringEdges(node_2_coord[0], node_2_coord[1], r=0.0001)
        node_1_ne = len(node_1_ne)
        node_2_ne = len(node_2_ne)
        self.netdict[edge_id] = (int(node_1_ne) + int(node_2_ne))
        return

    def calculateEdgeLengths(self, edge_id):
        get_edge = self.net.getEdge(edge_id)
        get_length = get_edge.getLength()
        self.netdict[edge_id] = [self.netdict[edge_id], get_length]
        return
    ...
    def intervalData(self):
        ...
        for edge in self.edges:
            ...
            emissions = self.getEmissions(current_edge) # Output Y variable
            mean_speed = self.getEdgeSpeed(current_edge) # Data we would be able to get from waze
            estimated_travel_time = self.getEdgeTravelTime(current_edge) # Data we would able to predict from waze
            traffic_level = self.getEdgeDensity(current_edge) # Data we would be able to get from waze
            values = self.netdict[current_edge]
            neighbours = values[0]
            length = values[1]

            current_row = [current_edge, emissions[0], emissions[1],
                    emissions[2], emissions[3], mean_speed, estimated_travel_time,
                    traffic_level, neighbours, length, self.step]
